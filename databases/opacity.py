...
def load_atomic_opacities(
    ...
    """Load atomic opacities using native ExoJAX AdbKurucz/AdbVald.

    Source priority:
        1. Cached .zarr opacity (if opa_load=True)
        2. Kurucz gfall (auto-download if enabled)
        3. VALD3 extract file (manual download)
    ...
    from config.paths_config import DB_KURUCZ, DB_VALD, DB_EXOATOM
    ...
    print("Loading atomic line databases (Kurucz/VALD/ExoAtom)...")
    ...
    for atom in atomic_species.keys():
        ...
        if snapshot is None and db_exoatom is not None:
            try:
                element, ionization = parse_species(atom)
                exoatom_name = element if ionization == 0 else atom
                mdb = MdbExomol(
                    exoatom_name,
                    nu_grid,
                    gpu_transfer=False,
                    local_databases=str(db_exoatom),
                )
        ...
        if snapshot is None:
            reason = []
            if kurucz_err is not None:
                reason.append(f"Kurucz: {kurucz_err}")
            if vald_file is None:
                reason.append("VALD: no extract file found")
            elif vald_err is not None:
                reason.append(f"VALD: {vald_err}")
            if exoatom_err is not None:
                reason.append(f"ExoAtom: {exoatom_err}")
            print(f"  * {atom} (skipping - {'; '.join(reason)})")
            continue
