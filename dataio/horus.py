...
def remove_doppler_shadow(
    ...
    shadow_profiles, baseline = compute_doppler_shadow(
        ...
        lambda_angle=planet_params.get('lambda_angle', config.DEFAULT_LAMBDA_ANGLE),
        b=planet_params['b'],
        ...
        period=planet_params.get('period', config.DEFAULT_PERIOD_DAY),
    ...
    if fit_scaling:
        ...
        if shadow_dot_shadow > 0:
            ...
            scale = config.DEFAULT_SHADOW_SCALING

        fit_info['scaling'] = scale
        shadow_model = scale * shadow_profiles
    else:
        fit_info['scaling'] = config.DEFAULT_SHADOW_SCALING
...
def fit_doppler_shadow(
    ...
    def residual_func(params, ccf_data, v_grid, phase, planet_params, stellar_params, fit_params):
        ...
        scaling = config.DEFAULT_SHADOW_SCALING
        ...
        shadow, _ = compute_doppler_shadow(
            phase=phase,
            vsini=sp['vsini'],
            lambda_angle=pp.get('lambda_angle', config.DEFAULT_LAMBDA_ANGLE),
            b=pp['b'],
            rp_rs=pp['rp_rs'],
            gamma1=sp['gamma1'],
            gamma2=sp['gamma2'],
    ...
    for pname in fit_params:
        if pname == 'scaling':
            x0.append(config.DEFAULT_SHADOW_SCALING)
        elif pname == 'lambda_angle':
            x0.append(planet_params.get('lambda_angle', config.DEFAULT_LAMBDA_ANGLE))
        elif pname == 'b':
            x0.append(planet_params['b'])
        elif pname == 'vsini':
            x0.append(stellar_params['vsini'])
        else:
            x0.append(planet_params.get(pname, stellar_params.get(pname, config.DEFAULT_FIT_PARAM_FALLBACK)))
